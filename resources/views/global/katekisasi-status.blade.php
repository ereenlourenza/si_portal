...
<div class="container py-8 max-w-5xl mx-auto">
    <h2 class="text-2xl md:text-3xl font-bold text-amber-700 mb-6 bg-amber-100 px-4 py-4 rounded shadow text-center md:text-left">
        Status Pendaftaran Katekisasi
    </h2>

    {{-- FORM FILTER --}}
    <form method="GET" action="{{ url()->current() }}" class="mb-6 px-3 flex flex-col md:flex-row items-center gap-4">
        <input type="text" name="q" value="{{ request('q') }}" placeholder="Masukkan nama katekumen..."
            ...
            @if($data->count())
                <table class="w-full table-auto border border-gray-200">
                    <thead class="bg-[#614D24] hover:bg-[#4f3f1c] text-white text-left text-sm">
                        <tr>
                            <th class="p-3 border border-[#614D24] text-center">Nama Katekumen</th>
                            <th class="p-3 border border-[#614D24] text-center">Is Baptis</th>
                    ...
                    <tbody class="text-sm">
                        @foreach($data as $item)
                            <tr class="hover:bg-amber-50">
                                <td class="p-3 border-[#614D24] text-center">{{ $item->nama_lengkap }}</td>
                                <td class="p-3 border-[#614D24] text-center">{{ $item->is_baptis }}</td>
