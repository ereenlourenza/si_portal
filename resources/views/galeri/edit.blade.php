...
@section('content')
    <div class="card card-outline card-primary">
        ...
        <div class="card-body">
            ...
            @else
                <form method="POST" action="{{ url('pengelolaan-informasi/galeri/'.$galeri->galeri_id) }}" class="form-horizontal" enctype='multipart/form-data'>
                    @csrf {!! method_field('PUT') !!} <!-- tambahkan baris ini untuk proses edit yang butuh method PUT -->
                    <div class="form-group row">
                        <label class="col-md-1 control-label col-form-label">Kategori Galeri<span class="text-danger">*</span></label>
                        <div class="col-md-11">
                            <select class="form-control" id="kategorigaleri_id" name="kategorigaleri_id" required>
                                <option value="" disabled>Pilih Kategori</option>
                                    @foreach($kategorigaleri as $item)
                                        <option value="{{ $item->kategorigaleri_id }}" 
                                            @if($item->kategorigaleri_id == $galeri->kategorigaleri_id) selected 
                                            @endif>{{ $item->kategorigaleri_nama }}
                                        </option>
                                    @endforeach
                            </select>

                            @error('kategorigaleri_id')
                    ...
                    <div class="form-group row align-items-center">
                        ...
                        <div class="col-md-11">
                            <input type="text" class="form-control" id="deskripsi" name="deskripsi" value="{{ old('deskripsi', $galeri->deskripsi) }}">
                            @error('deskripsi')
                                <small class="form-text text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-11">
                                ...
                                @if($galeri->foto)
                                    <div class="mb-2">
                                        ...
                                        <a href="{{ Storage::url('images/galeri/' . $galeri->foto) }}" target="_blank" class="btn btn-info btn-sm">
                                            <i class="nav-icon far fa-eye"></i> Lihat Foto
                                        </a>
                                    </div>
                                @endif   
                                <input type="file" class="form-control" id="foto" name="foto" accept=".jpg,.jpeg,.png" value="{{ old('foto', $galeri->foto) }}">
                    ...
                    <div class="form-group row align-items-center">
                        ...
                        <div class="col-md-11">
                            ...
                            <a class="btn btn-sm btn-default ml-1" href="{{ url('pengelolaan-informasi/galeri') }}">Kembali</a>
                        </div>
                    </div>
                </form>
            @endempty
